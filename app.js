...
function card(g, mini){
  ...
  el.addEventListener('click', () => { if(!suppressClick) tapToggle(g); });
  enableDrag(el, g);
  return el;
}
...
function cardRE(g){
  ...
  el.addEventListener('click', () => {
    ...
    else { reDone.add(g.id); toast(`✔ ${g.title} concluído!`); }
...
function tapToggle(g){
  if(st(g.id) === 'done'){ delete status[g.id]; }
  else { status[g.id] = 'done'; toast(`✔ ${g.title} zerado!`); }
...
function render(){
  ...
  if(filter === 're'){
    ...
    shelf.hidden = true;
    const list = BUY_GAMES.filter(g => !bought.has(g.id));
    grid.innerHTML = '';
...
function updateStats(){
  ...
  if(filter === 're'){
    ...
    total = BUY_GAMES.length;
    d = bought.size;
    const invest = BUY_GAMES.filter(g => !bought.has(g.id))
